...
def save_segment_labels(video_path: Path, labels: List[int]) -> None:
    ...
    segments = []
    start = 0
    current = labels[0] if labels else 0
...
def draw_overlay(
    ...
    text_lines = [
        ...
        f"Phase: {current_phase_id} - {current_phase_name}",
        f"Labeled frames: {assigned_count}/{total_frames}",
        "",
        "Controls:",
        "a/d: prev/next frame",
        "p: play/pause",
    ...
    cv2.rectangle(overlay, (10, 10), (440, 420), (0, 0, 0), -1)
    alpha = 0.65
...
def annotate_video(video_path: Path) -> None:
    ...
    window_name = "Snatch Phase Annotator"
    ...
    while True:
        ...
        if key == ord("q"):
            print("[INFO] Saliendo sin guardar automáticamente.")
            break
            ...
            phase_id = int(chr(key))
            labels[frame_idx] = phase_id

            # avance automático al siguiente frame tras etiquetar
            if frame_idx < total_frames - 1:
                frame_idx += 1
