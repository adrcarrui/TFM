...
FRAME_LABELS_DIR = Path("data/annotations/frame_labels")
SEGMENT_LABELS_DIR = Path("data/annotations/segment_labels")
...
def save_frame_labels(video_path: Path, labels: List[int]) -> None:
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["video", "frame", "phase_id", "phase_name"])
...
def save_segment_labels(video_path: Path, labels: List[int]) -> None:
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["video", "start_frame", "end_frame", "phase_id", "phase_name"])
...
def clear_range(labels: List[int], start_idx: int, end_idx: int) -> None:
    assign_range(labels, start_idx, end_idx, 0)


def summarize_segments(labels: List[int], max_items: int = 10) -> List[str]:
    segments = build_segments(labels)
    lines = []
...
def draw_overlay(
    ...
    right_panel_lines.extend(summarize_segments(labels, max_items=12))

    overlay = display.copy()
    cv2.rectangle(overlay, (10, 10), (470, 560), (0, 0, 0), -1)
    cv2.rectangle(overlay, (w - 330, 10), (w - 10, 320), (0, 0, 0), -1)
    alpha = 0.62
    ...
    # pintar rango seleccionado sobre la barra
    ...
    # si el frame actual está dentro del rango, pintar borde rojo
    if range_start is not None and range_end is not None:
        lo = min(range_start, range_end)
        hi = max(range_start, range_end)
        if lo <= frame_idx <= hi:
...
def annotate_video(video_path: Path) -> None:
    ...
    window_name = "Snatch Phase Annotator - Range Mode"
    ...
    while True:
        ...
        if key == ord("q"):
            ...
            if range_start is not None and range_end is not None:
                clear_range(labels, range_start, range_end)
                print(f"[INFO] Rango limpiado: {min(range_start, range_end)}-{max(range_start, range_end)}")
            range_start = None
            range_end = None
            ...
            if range_start is not None and range_end is not None:
                assign_range(labels, range_start, range_end, selected_phase)
                lo = min(range_start, range_end)
                hi = max(range_start, range_end)
